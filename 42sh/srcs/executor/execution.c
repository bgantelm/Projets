...
int		handle_connect(t_connect *connect)
{
	if (connect->type == R_SIMPLE && handle_r_simple(connect) == -1)
		return (-1);
	else if (connect->type == R_DOUBLE && handle_r_double(connect) == -1)
		return (-1);
	else if (connect->type == L_SIMPLE && handle_l_simple(connect) == -1)
		return (-1);
	return (0);
}
